...
#warning Mouse Is Not Currently Supported
    public class PlayerCameraController : MonoBehaviour, IPlayerCameraController
    {
        ...
        public MapPos MapCursor
        {
            ...
            set
            {
                if(!m_voxelMap.MapBounds.Contains(value))
                {
                    throw new System.ArgumentException("out of map bounds", "value");
                }
                SetMapCursor(value);
                UpdateVirtualMousePosition();
            }
        ...
        private void Start()
        {
            ...
            foreach(long unit in units)
            {
                ...
                if(VoxelData.IsControllableUnit(dc.ControlledData.Type))
                {
                    ...
                    Debug.Assert(m_voxelMap.MapBounds.Contains(MapCursor));
        ...
        private void CreateCursor()
        {
            ...
            Debug.Assert(m_voxelMap.MapBounds.Contains(MapPivot));
        ...
        private void Update()
        {
            ...
            if(IsInputEnabled)
            {
                ...
                yAxis = m_inputManager.GetAxisRaw(InputAction.MoveForward, LocalPlayerIndex);
                xAxis = m_inputManager.GetAxisRaw(InputAction.MoveSide, LocalPlayerIndex);


                //Vector3 screenVector = new Vector3(
                //    m_inputManager.GetAxisRaw(InputAction.MoveSide, LocalPlayerIndex),
                //    0,
                //    m_inputManager.GetAxisRaw(InputAction.MoveForward, LocalPlayerIndex));

                //screenVector.Normalize();

                //Vector3 worldVector = m_camera.cameraToWorldMatrix.MultiplyVector(screenVector);
                //worldVector.y = 0;
        ...
        private void MoveCursor(float xAxis, float yAxis, PlayerCamCtrlSettings settings)
        {
            ...
            if(m_moveCursorDelay <= 0)
            {
                ...
                MapPos newMapCursor = new MapPos(MapCursor.Row + deltaRow, MapCursor.Col + deltaCol);

                if(m_voxelMap.MapBounds.Contains(newMapCursor))
                {
                    SetMapCursor(newMapCursor);
        ...
        private void MoveCursorUsingMouse(PlayerCamCtrlSettings settings)
        {
            ...
            if(m_voxelMap.MapBounds.Contains(cursorPos))
            {
        ...
        private void MovePivotPoint(float xAxis, float yAxis, bool outOfScreen, PlayerCamCtrlSettings settings)
        {
            ...
            if (m_movePivotDelay <= 0)
            {
                ...
                Vector3 offset = m_camera.cameraToWorldMatrix.MultiplyVector(screenVector);
                offset.y = 0;
                offset.Normalize();

                int deltaRow = Mathf.RoundToInt(offset.z);
                int deltaCol = Mathf.RoundToInt(offset.x);
        ...
        private void FixMapCursor()
        {
            for (int r = 0; r <= 50; ++r)
            {
                for (int i = -1; i <= 1; ++i)
                {
                    for (int j = -1; j <= 1; ++j)
                    {
                        MapPos newMapCursor = new MapPos(MapCursor.Row + i * r, MapCursor.Col + j * r);
                        if(m_voxelMap.MapBounds.Contains(newMapCursor))
                        {
                            SetMapCursor(newMapCursor);
                            if (WillBeOutOfScreen(TargetCursor))
